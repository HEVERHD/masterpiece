      ...
      >
        ...
        {step === "success" && (
          <div className="p-8 text-center max-w-sm mx-auto space-y-4">
            ...
            {deliveryType === "tienda" && (
              <p className="text-xs text-stone-500 bg-stone-50 rounded-xl p-3">📍 {STORE_ADDRESS}</p>
            )}
            {deliveryType === "envio_nacional" && (
            ...
            <a
              href={`https://wa.me/${process.env.NEXT_PUBLIC_ADMIN_WHATSAPP ?? "573150014381"}?text=${encodeURIComponent(
                `Hola Masterpiece! 👋 Adjunto mi comprobante de pago.\n\n` +
                `Pedido: *${product.name}*${selectedSize ? ` — ${selectedSize}` : ""}\n` +
                (orderId ? `ID: ${orderId}` : "")
              )}`}
              target="_blank"
            ...
            <button onClick={handleClose} className="text-sm text-stone-400 hover:text-stone-600 transition-colors">
